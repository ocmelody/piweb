...
#      Provides a straightforward way of streaming camera images from the      #
#      Raspberry Pi camera. HTTP server ripped rather crudely from             #
#      MJPG-streamer                                                           #
#                                                                              #
#      Copyright (C) 2014 Alan Broun - Dawn Robotics Ltd                       #
...
//extern HttpServerContext servers[MAX_OUTPUT_PLUGINS];
...
int _read(int fd, iobuffer *iobuf, void *buffer, size_t len, int timeout)
{
    ...
    while((copied < len)) {
        ...
        memcpy((uint8_t*)buffer + copied, (uint8_t*)iobuf->buffer + IO_BUFFER - iobuf->level, i);
...
Return Value: -
******************************************************************************/
void send_snapshot( cfd* pCfd, int input_number, bool send_small_image=false )
{
    unsigned char *frame = NULL;
    int frame_size = 0;
    uint8_t curWriteSlotIdx = 0;
    uint8_t curReadSlotIdx = 0;
    
    char buffer[BUFFER_SIZE] = {0};
    struct timeval timestamp;

    // Work out which frame to use and get the frame size and data
    if ( send_small_image )
    {
        curWriteSlotIdx = *(pCfd->pc->mpCurWriteSmallImageSlotIdx);
        curReadSlotIdx = ( curWriteSlotIdx == 0 ? pCfd->pc->mNumSmallImageSlots - 1 : curWriteSlotIdx - 1 );
        
        frame = &(pCfd->pc->mSmallImageSlots[ curReadSlotIdx*pCfd->pc->mMaxSmallImageSize ]);
        frame_size = pCfd->pc->mSmallImageSizes[ curReadSlotIdx ];
    }
    else
    {
        curWriteSlotIdx = *(pCfd->pc->mpCurWriteImageSlotIdx);
        curReadSlotIdx = ( curWriteSlotIdx == 0 ? pCfd->pc->mNumImageSlots - 1 : curWriteSlotIdx - 1 );
        
        frame = &(pCfd->pc->mImageSlots[ curReadSlotIdx*pCfd->pc->mMaxImageSize ]);
        frame_size = pCfd->pc->mImageSizes[ curReadSlotIdx ];
    }
    ...
    DBG( "Trying to write %i bytes\n", frame_size );
    
    if( write(pCfd->fd, buffer, strlen(buffer) ) < 0 )
    {
        DBG( "Error: Unable to write header\n" );
        DBG( "Header is\n" );
        DBG( "%s\n", buffer );
        return;
    }
    
    if( write(pCfd->fd, frame, frame_size ) < 0 )
    {
        DBG( "Error when writing frame\n" );
        DBG( "Errno is %i\n", errno );
        return;
    }
}
...
/******************************************************************************
Description.: Send a complete HTTP response and motion vectors for a single frame.
Input Value.: file descriptor fd to send the answer to
Return Value: -
******************************************************************************/
void send_motion_vectors_snapshot( cfd* pCfd, int input_number)
{
    unsigned char *frame = NULL;
    int frame_size = 0;
    char buffer[BUFFER_SIZE] = {0};
    ...
    uint8_t curWriteSlotIdx = *(pCfd->pc->mpCurWriteMotionVectorsSlotIdx);
    uint8_t curReadSlotIdx = ( curWriteSlotIdx == 0 
        ? pCfd->pc->mNumMotionVectorsSlots - 1 : curWriteSlotIdx - 1 );
    ...
    sprintf(buffer, "HTTP/1.0 200 OK\r\n" \
            STD_HEADER \
            "Content-type: application/x-motion-vectors\r\n" \
            "X-Timestamp: %d.%06d\r\n" \
            "\r\n", (int) timestamp.tv_sec, (int) timestamp.tv_usec);

    /* send header and motion vectors now */
    frame = &(pCfd->pc->mMotionVectorsSlots[ curReadSlotIdx*pCfd->pc->mMaxMotionVectorsSize ]);
    frame_size = pCfd->pc->mMotionVectorsSizes[ curReadSlotIdx ];
...
int timeval_subtract( const timeval& a, const timeval& b, timeval* pResultInOut )
{
    timeval x = a;
    timeval y = b;
    ...
    pResultInOut->tv_sec = x.tv_sec - y.tv_sec;
    pResultInOut->tv_usec = x.tv_usec - y.tv_usec;
...
Return Value: -
******************************************************************************/
void send_stream( cfd* pCfd, int input_number, bool send_small_image=false )
{
    unsigned char *frame = NULL;
    int frame_size = 0;
    uint8_t curWriteSlotIdx = 0;
    ...
    if(send(pCfd->fd, buffer, strlen(buffer), MSG_NOSIGNAL) < 0) {
        DBG("Error, unable to send header\n");
        return;
    }

    DBG("Headers send, sending stream now\n");

    timeval lastTestTime;
    gettimeofday( &lastTestTime, NULL );
    
    int numFramesSentInLastSecond = 0;
    
    while( true ) //!pglobal->stop) 
    {
        // Work out which frame to use and get the frame size and data
        if ( send_small_image )
        {
        ...
        timeval timeDiff;
        timeval_subtract( timestamp, lastTestTime, &timeDiff );
        if ( timeDiff.tv_sec >= 1 )
        {
            //printf( "Sent %i frames in last second\n", numFramesSentInLastSecond );
            numFramesSentInLastSecond = 0;
        ...
        if(send(pCfd->fd, buffer, strlen(buffer),MSG_NOSIGNAL) < 0) break;

        DBG("sending frame\n");
        if(send(pCfd->fd, frame, frame_size,MSG_NOSIGNAL) < 0) break;
        ...
        // Sleep to acheive the desired frame rate
        int microSecsPerFrame = 1000 * 1000 / pCfd->pc->mWebServerFPS;
        
        timeval frameEndTime;
        gettimeofday( &frameEndTime, NULL );
        timeval_subtract( frameEndTime, timestamp, &timeDiff );
        
        if ( timeDiff.tv_sec == 0 && timeDiff.tv_usec < microSecsPerFrame )
        {
            //printf( "Sleeping for %i microsecs\n", microSecsPerFrame - timeDiff.tv_usec );
            usleep( microSecsPerFrame - timeDiff.tv_usec );
        }
    }
}
...
Return Value: -
******************************************************************************/
void send_error(int fd, int which, const char *message)
{
    char buffer[BUFFER_SIZE] = {0};
...
#endif
        input_suffixed = 255;
    } else if(strstr(buffer, "GET /small_image/?action=snapshot") != NULL) {
        req.type = A_SMALL_SNAPSHOT;
    } else if(strstr(buffer, "GET /small_image/?action=stream") != NULL) {
        req.type = A_SMALL_STREAM;
    } else if(strstr(buffer, "GET /motion_vectors/?action=snapshot") != NULL) {
        req.type = A_MOTION_VECTORS_SNAPSHOT;
...
//     }
    ...
    case A_STREAM:
        ...
        break;
    case A_SMALL_SNAPSHOT:
        DBG("Request for small snapshot from input: %d\n", input_number);
        send_snapshot( &lcfd, input_number, true );
        break;
    case A_SMALL_STREAM:
        DBG("Request for small stream from input: %d\n", input_number);
        send_stream( &lcfd, input_number, true );
        break;
    case A_MOTION_VECTORS_SNAPSHOT:
        DBG("Request for motion vectors snapshot from input: %d\n", input_number);
        send_motion_vectors_snapshot( &lcfd, input_number );
        break;
//     case A_COMMAND:
...
//         break;
    default:
        send_error(lcfd.fd, 501, "unknown request");
        DBG("unknown request\n");
    }
...
{
    HttpServerContext* pcontext = (HttpServerContext*)arg;
    int i;
